-- Phase B — seed 13 категорий РС поверх schema из `006_migrate_categories_rsbor.sql`.
-- Применяется после 006 (требует колонки rsbor_id и icon_path).
-- Идемпотентна: все UPDATE'ы и UPSERT'ы можно прогонять повторно.
